...
impl Account {
	...
	pub async fn new(username: &String, password: &String, account_type: &AccountType) -> Result<Self, sqlx::Error> {

		let password_hash = crypto::encode_password(password);
		
		sqlx::query_as!(
			Account,
			r#"INSERT INTO account (username, password_hash, account_type) VALUES ($1, $2, $3) RETURNING uuid, username, password_hash, profile_picture_uuid, account_type AS "account_type!: AccountType""#,
			username,
			password_hash,
